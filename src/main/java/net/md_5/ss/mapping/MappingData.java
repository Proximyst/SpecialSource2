...
public class MappingData {
  ...
  public String printUnused() {
    ...
    AccessTrackingMap[] aaccesstrackingmap = new AccessTrackingMap[]{this.packageMaps, this.classMaps, this.methodMaps,
        this.fieldMaps};
    ...
    for (int j = 0; j < i; ++j) {
      ...
      while (iterator.hasNext()) {
        ...
        if (!section.isSeen(entry.getValue())) {
          sb.append("Unseen: ").append((String) entry.getKey()).append(" -> ").append((String) entry.getValue())
              .append("\n");
        }
      }
    }
