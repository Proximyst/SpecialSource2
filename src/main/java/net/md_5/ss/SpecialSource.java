...
public class SpecialSource {
  ...
  private static OptionParser map(String[] args) throws IOException {
    OptionParser parser = new OptionParser() {
      {
        ...
        this.acceptsAll(Arrays.asList("m", "mappings"), "Mappings").withRequiredArg().ofType(File.class)
            .withValuesSeparatedBy(',');
        this.acceptsAll(Arrays.asList("e", "excludes"), "Excludes").withRequiredArg().ofType(File.class);
        this.acceptsAll(Arrays.asList("auto-lvt"), "Fix LVT automatically in the specified style").withRequiredArg()
            .ofType(EnhancedMethodRemapper.LVTStyle.class);
        this.acceptsAll(Arrays.asList("auto-member"), "Automatically generate names for the given member types")
            .withRequiredArg().ofType(EnhancedRemapper.AutoMember.class);
        this.acceptsAll(Arrays.asList("only"), "Map only the specified packages").withRequiredArg()
            .ofType(String.class);
      }
    };
    OptionSet set = parser.parse(args);
    File inJar = (File) set.valueOf("in-jar");
    AggregateRepo inRepo = new AggregateRepo(
        new ClassRepo[]{new JarRepo(new JarFile(inJar)), RuntimeRepo.getInstance()});
    ...
    while (iterator.hasNext()) {
      ...
      EnhancedRemapper remapper = new EnhancedRemapper(inRepo, mappingData, set.valuesOf("auto-member"),
          (EnhancedMethodRemapper.LVTStyle) set.valueOf("auto-lvt"));
